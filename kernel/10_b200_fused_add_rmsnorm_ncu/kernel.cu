...
__global__ __launch_bounds__(512, 4) void fused_add_rmsnorm(
  ...
  const uint4 h0_raw = hidden[index0];
  const uint4 r0_raw = residual[index0];
  const uint4 h1_raw = hidden[index1];
  const uint4 r1_raw = residual[index1];
  const uint4 w0_raw = weight[threadIdx.x];
  const uint4 w1_raw = weight[threadIdx.x + 512];

  const Bf16x8 h0 = as_bf16x8(h0_raw);
  const Bf16x8 r0 = as_bf16x8(r0_raw);
  const Bf16x8 h1 = as_bf16x8(h1_raw);
  const Bf16x8 r1 = as_bf16x8(r1_raw);
  const Bf16x8 w0 = as_bf16x8(w0_raw);
  const Bf16x8 w1 = as_bf16x8(w1_raw);
...
#pragma unroll
  for (int i = 0; i < 4; ++i) {
    ...
    square_sum0 = fmaf(x0.x, x0.x, square_sum0);
    square_sum0 = fmaf(x0.y, x0.y, square_sum0);

    sum1.value[i] = __hadd2(h1.value[i], r1.value[i]);
    const float2 x1 = __bfloat1622float2(sum1.value[i]);
    square_sum1 = fmaf(x1.x, x1.x, square_sum1);
    square_sum1 = fmaf(x1.y, x1.y, square_sum1);
  }
...
__global__ __launch_bounds__(1024, 1) void fused_add_rmsnorm_t1024(
  ...
  const uint4 h_raw = hidden[index];
  const uint4 r_raw = residual[index];
  const uint4 w_raw = weight[threadIdx.x];

  const Bf16x8 h = as_bf16x8(h_raw);
  const Bf16x8 r = as_bf16x8(r_raw);
  const Bf16x8 w = as_bf16x8(w_raw);
  Bf16x8 sum;
  float square_sum0 = 0.0f;
  float square_sum1 = 0.0f;
...
#pragma unroll
  for (int i = 0; i < 4; ++i) {
    ...
    if (i < 2) {
      square_sum0 = fmaf(x.x, x.x, square_sum0);
      square_sum0 = fmaf(x.y, x.y, square_sum0);
    } else {
      square_sum1 = fmaf(x.x, x.x, square_sum1);
      square_sum1 = fmaf(x.y, x.y, square_sum1);
    }
  }
